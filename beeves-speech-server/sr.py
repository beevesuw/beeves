...
class AudioFile(AudioSource):
    ...
    def __enter__(self):
        ...
        try:
            ...
            try:
                ...
                self.audio_reader = aifc.open(
                    self.filename_or_fileobject, "rb")
                ...
                if hasattr(self.filename_or_fileobject, "read"):
                    ...
                    with open(self.filename_or_fileobject,
                              "rb") as f: flac_data = f.read()
        ...
        if self.SAMPLE_WIDTH == 3:  # 24-bit audio
            ...
            try: audioop.bias(b"", self.SAMPLE_WIDTH, 0)
    ...
    class AudioFileStream(object):
        ...
        def read(self, size=-1):
            ...
            if not isinstance(buffer, bytes): buffer = b""
...
class AudioData(object):
    ...
    def get_segment(self, start_ms=None, end_ms=None):
        ...
        if end_ms is None:
            ...
            end_byte = int((end_ms * self.sample_rate *
                           self.sample_width) // 1000)
    ...
    def get_raw_data(self, convert_rate=None, convert_width=None):
        ...
        assert convert_width is None or (convert_width % 1 == 0 and 1 <= convert_width <=
                                         4), "Sample width to convert to must be between 1 and 4 inclusive"
    ...
    def get_flac_data(self, convert_rate=None, convert_width=None):
        ...
        """
        assert convert_width is None or (convert_width % 1 == 0 and 1 <= convert_width <=
                                         3), "Sample width to convert to must be between 1 and 3 inclusive"
    ...
    def get_ogg_opus_data(self, convert_rate=16000, profile='audio'):
        ...
        convert_width = 2  # the largest supported sample width is `6`-bit, so we'll limit the sample width to that
        # run the FLAC converter with the WAV data to get the FLAC data
        raw_data = self.get_raw_data(16000, 2)
        opus_converter = get_opusenc()
        if os.name == "nt":  # on Windows, specify that the process is to be started without showing a console window
            startup_info = subprocess.STARTUPINFO()
            # specify that the wShowWindow field of `startup_info` contains a value
            startup_info.dwFlags |= subprocess.STARTF_USESHOWWINDOW
        ...
        process = subprocess.Popen([
            ...
            # put the resulting AIFF file in stdout, and make sure it's not mixed with any program output
            "--raw",
            "--raw-rate", "16000",  # decode the FLAC file into an AIFF file
            "-",  # the input FLAC file contents will be given in stdin,
            '-'
        ], stdin=subprocess.PIPE, stdout=subprocess.PIPE, startupinfo=startup_info)
        opus_data, stderr_data = process.communicate(raw_data)
        return opus_data
...
class Recognizer(AudioSource):
    ...
    def recognize_speaktome(self, audio_data, language="en-US", endpoint_url='https://speaktome-2.services.mozilla.com/'):
        """
        Performs speech recognition on ``audio_data`` (an ``AudioData`` instance), using the IBM Speech to Text API.
        ...
        #raw_data = self.get_raw_data(convert_rate=16000, convert_width=2)
        opus_data = audio_data.get_ogg_opus_data()
        with open('last.opus', 'wb') as of:
            of.write(opus_data)
        ...
        mozcloud_headers = {
            ...
            'Accept-Language-STT': 'en-US',
            'Store-Sample': '0',
            'Store-Transcription': '0',
...
def get_opusenc():
    """Returns the absolute path of a FLAC converter executable, or raises an OSError if none can be found."""
    opusenc = shutil_which("opusenc")  # check for installed version first
    if opusenc is None:  # flac utility is not installed
        raise OSError(
            "opusenc conversion utility not available - consider installing the opusenc command line application by running `apt-get install opusenc` or your operating system's equivalent")
    return opusenc
